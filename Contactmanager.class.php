...
class Contactmanager extends FreePBX_Helpers implements BMO {
	...
	*/
	...
	public function addGroup($name, $type='internal', $owner = -1, $updateContactFile = true) {
		if (!$name || empty($name)) {
			return array("status" => false, "type" => "danger", "message" => _("Group name is required and must not contain special characters."));
		}
	...
	public function deleteEntriesByGroupID($groupid, $updateContactFile = true) {
		...
		$sql = "select id FROM contactmanager_group_entries WHERE `groupid` = :groupid";
		$sth = $this->db->prepare($sql);
		$sth->execute(array(':groupid' => $groupid));
